...
def combine(resource, doc, env, *args, **kwargs):
    ...
    * METATAB_PACKAGE
    ...
    suffix = resource.name.split('_')[-1]
    headers = None
    for i, r in enumerate(doc.resources()):
        
        # Iter only '*_lea' files for the upc_combined_lea, etc. 
        if suffix not in r.name or 'combined' in r.name or int(r.year) < 2014:
            continue

        if not headers:
            headers =  ['file_year']+ r.headers
            yield headers
